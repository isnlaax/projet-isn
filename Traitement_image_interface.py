...
if sys.platform == 'win32':
    plugdir = False
    for p in [ ( 'USERPROFILE', '\\Desktop\\filtres', '' ),
               ( 'HOMEPATH', '\\Desktop\\filtres', '' ),
               ( 'SystemDrive', 'filtres', '' ),
               ( 'USERPROFILE', '\\Desktop', 'filtres' ),
               ( 'HOMEPATH', '\\Desktop', 'filtres' ),
               ( 'SystemDrive', '', 'filtres' ) ]:
    ...
    plugdir = os.environ['HOME'] + '/filtQSDres'
...
def modrel():
    ...
    for b in buts:
        b.destroy()
    (ncol, buts) = loadmods();
    ilab.grid(column=0, row=0, rowspan=12)
...
def Reload():
    global  fn,pic
    pic = ImageBender(file=fn)
    ilab.configure(image=pic)


# Menu bar contents
fmenu = Menu(top, bg=bgcolor, activebackground=abgcolor)
fmenu.add_command(label='Ouvrir..', command=getfile, underline=0)
fmenu.add_command(label='Sauver...', command=savefile, underline=0)
fmenu.add_command(label='Sortir', command=quit, underline=0)
#fmenu.add_command(label='annuler',command=undo, underline=0)
top.add_cascade(label='File', menu=fmenu, underline=0)


Bouton1=Button(bgframe,text='ANNULER',bg=bgcolor,activebackground=abgcolor,command=Reload)  #font=('arial',10),
Bouton1.grid(row=11,column=1,sticky=N)


pmenu = Menu(top, bg=bgcolor, activebackground=abgcolor)
pmenu.add_command(label='Charger', command=modrel, underline=0)
pmenu.add_command(label='Localisation...', command=setmodloc, underline=0)
top.add_cascade(label='Filtres', menu=pmenu, underline=0)

# Display the image
ilab = Label(bgframe, image=pic)
ilab.grid(row=0, column=0, rowspan=12)      #columnspan=ncol)
